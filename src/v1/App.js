...
class App extends Component {
  ...
  saveItem = async e => {
    ...
    const r = await fetch(`//dumpster-api.sixfstudios.now.sh?url=${url}`);
          ...
          {this.state.canEdit && (
            <div
              style={{
                ...style.topContainer,
                marginTop:
                  collectionHasItems && window.innerWidth < 800 ? 40 : 0,
            ...
            >
              {!collectionHasItems && <Logo />}
              <input
                placeholder={`Paste a link to ${
                  collectionHasItems || this.state.collectionId
            ...
            {map(this.state.items, (item, id) => {
                ...
                return (
                  <Album
                    ...
                    handleRemoveImage={url =>
                      this.removeImageFromItem(url, item)
                    }
                  />
                );
              }
              return (
                <Image
                  key={id}
                  editable={this.state.canEdit}
                  handleRemove={url => this.removeImageFromItem(url, item)}
